...
InstructionSet = namedtuple(
    "InstructionSet", ['opcode', 'pos1', 'pos2', 'dest_pos'])

InstructionSet.__new__.__defaults__ = (None,) * len(InstructionSet._fields)
...
def run(data, debug=False):
    for instructions in map(
        ...
        (x for x in (data[i: i + 4] for i in range(0, len(data),
                                                   4)))):
        if debug:
            print("Current instruction set: {}".format(instructions))
        if instructions.opcode == 1:
            data[instructions.dest_pos] = data[instructions.pos1] + data[instructions.pos2]
            if debug:
                print("Addition complete. Current state: {}".format(data))
        elif instructions.opcode == 2:
            data[instructions.dest_pos] = data[instructions.pos1] * data[instructions.pos2]
            if debug:
                print("Multiplication complete. Current state: {}".format(data))
        elif instructions.opcode == 99:
            print("Process complete.\nFinal state: {}".format(data))
            break
        else:
            print(
                "Execution error! Unknown opcode: {}".format(
                    instructions.opcode))
